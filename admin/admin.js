...
$("#loginForm").addEventListener("submit", async e => {
  ...
  if (card) card.classList.remove("shake-card");
  ...
  try {
    await api("/api/admin/login", { method: "POST", body: JSON.stringify({ password: pwdInput.value.trim() }) });
    showApp();
  } catch (err) {
    if (errorEl) errorEl.textContent = "✕ " + (err.message || "Invalid admin password");
    if (card) {
    ...
    if (submitBtn) {
      ...
      submitBtn.innerHTML = `<span class="btn-text">Unlock Dashboard &rarr;</span>`;
    }
  }
});
